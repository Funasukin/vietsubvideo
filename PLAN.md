...
- [x] **Phase 1.10 — thumbnail tự động + S9 metadata hoàn chỉnh** *(2026-06-11)*: `core/thumbnail.py` — chọn frame tại khoảng lặng giữa các câu thoại (né phụ đề cháy trong hình), cv2 chấm điểm nét/sáng/màu, Claude vision chọn frame đắt nhất, PIL vẽ 2 dòng hook (tự co cỡ chữ chống tràn). S9 sinh metadata.json (title/description/tags/hook) bằng `METADATA_MODEL` (mặc định Sonnet — title là mặt tiền kênh, chỉ ~$0.02/job). Dashboard hiện thumbnail + nút tạo lại. Kit đăng tay đủ bộ: final.mp4 + sub_vi.srt + metadata.json + thumbnail.jpg.
- [x] **Phase 1.11 — batch/playlist** *(2026-06-11)*: dán nhiều link hoặc link playlist/kênh vào dashboard → `core/sources.py` bung danh sách bằng yt-dlp extract_flat (cap `BATCH_LIMIT`=50) → xác nhận số lượng → xếp hàng tuần tự. Chống trùng theo URL với job đã có; job pending được tự xếp lại hàng đợi khi server khởi động (batch dài sống sót qua restart).
...
từ chữ; ưu tiên đo trên vocals.wav (demucs) khi có.

**Cập nhật 2026-07-04 — MỨC 2 ĐÃ LÀM: A+B chung một mạch** (`core/emotion.py`,
bật/tắt EMOTION): Claude gắn nhãn cảm xúc từng câu khi dịch (gap/gian/buon/thitham,
field "emotion" schema S4) → edge-tts CỘNG offset rate/pitch/volume vào prosody mức 1
...
**Cập nhật 2026-07-04 — C/D ĐÃ TÍCH HỢP** (`core/paid_tts.py`): engine
elevenlabs | vbee | fpt trong dropdown "Giọng đọc" — key/token nhập tab Cấu hình
(che như bot token, UI không đọc ra được). ElevenLabs đọc đa ngôn ngữ (dùng cả khi
TARGET_LANG ≠ vi); VBee/FPT chỉ tiếng Việt (đích khác tự về edge). Câu cast
voice_ref vẫn viXTTS; prosody transfer (mức 3) vẫn áp lên output; S5 báo tổng ký
tự trước khi gửi (dịch vụ tính phí theo ký tự). Đây cũng là các engine AN TOÀN
...
8. ~~**Nhận diện người nói từ audio gốc** (pyannote, Phase 4)~~ — ĐÃ LÀM 2026-07-03
   (`core/speakers.py`: nhãn speaker vào batch dịch + giới tính theo cụm + engine
   viXTTS tự chia clip giọng theo cụm; bật DIARIZE=1 + HF_TOKEN, cần pyannote/GPU).
