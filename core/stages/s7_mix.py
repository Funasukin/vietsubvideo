...
def render_voice(job_dir, seg: dict, rate: int, slot: int, espeed: float,
    ...
    Logic DUY NHẤT — run() và /mix-preview cùng gọi (Codex: preview viết bản
    "gần giống" là mất lòng tin). sped_path=None → tts/seg_XXXX_sped.wav chuẩn;
    preview truyền path tạm riêng để không giẫm file của lần trộn thật."""
    mp3 = job_dir / "tts" / f"seg_{seg['id']:04d}.mp3"
    voice = _load_voice(mp3, rate)

    def _ms(n: int) -> int:
        return int(n * 1000 / rate)

    fade_n = int(duration.FADE_GUARD_S * rate)
    limit = max(int(0.3 * rate), slot - fade_n)
    ...
    if len(voice) > limit * duration.TOL:
        ...
        if factor > 1.004:
            ...
            ffmpeg.run("-i", str(mp3), "-filter:a", f"atempo={factor:.4f}", str(sped_path))
            voice = _load_voice(sped_path, rate)
        else:
            factor = 1.0
    elif stretch_short and len(voice) < int(0.7 * slot) and len(voice) < mouth:
    ...
    if len(voice) > slot:
        ...
        # câu kế / thoại gốc chưa duck như trước (V4).
...
def run(job: Job) -> None:
    ...
    for i, seg in enumerate(segments):
        ...
        if row["clipped_ms"] > 0:
            warnings.append({
                "id": seg["id"],
                "overflow_ms": row["clipped_ms"],  # giữ tên key cũ cho UI/QC — giờ = bị cắt
                "text_vi": seg["text_vi"],
            })
        detail.append(row)
